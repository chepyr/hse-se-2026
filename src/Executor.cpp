...
static CommandResult executePipelinePosix(
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (pid == 0) {
            ...
            // Use std::cin/cout/cerr which will use the redirected file descriptors
            IOStreams child_io{std::cin, std::cout, std::cerr};
            
            // Check for built-in (note: most builtins don't make sense in pipeline)
            // But we'll handle them anyway for completeness
