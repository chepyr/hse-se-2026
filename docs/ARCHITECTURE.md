...
Документ описывает целевую архитектуру проекта.
В рамках текущего этапа (HW3) реализованы:

- REPL
- Tokenizer с поддержкой кавычек
- Подстановка переменных `$VAR` и `${VAR}`
- Parser с поддержкой пайплайнов
- Executor с поддержкой pipe/fork/dup2
- Builtins внутри пайплайна
- Встроенная команда **grep** (регулярные выражения, -w, -i, -A; разбор опций через CLI11)
- External runner с передачей окружения
- Environment для хранения переменных окружения
...
- `grep` — поиск по регулярному выражению (опции: -w целое слово, -i без учёта регистра, -A N строк после совпадения; разбор аргументов через библиотеку CLI11);
...
Диаграммы приведены к нотации, совместимой с UML: компонентная структура, потоки управления и данных, конечные автоматы и структуры данных соответствуют принятым в UML представлениям (компоненты, активности, состояния, композиция). Исходные схемы в формате Mermaid вынесены в отдельный файл:
...
### 8.3 Команда grep и разбор опций (CLI11)

Команда `grep` имеет нетривиальный набор опций (-w, -i, -A N) и позиционных аргументов (pattern, [FILE...]). Разбор аргументов реализован через библиотеку **CLI11** (header-only, C++), а не вручную по `argv`, в соответствии с требованием задания.

- **Модуль Grep**: функция `runGrep(argv, io)` принимает уже разобранные shell-ом аргументы (вектор строк), строит из них `argc`/`argv` для CLI11 и вызывает `CLI::App::parse()`. После разбора опций выполняется поиск по регулярному выражению (std::regex), с учётом -w (граница слова через std::isalnum и '_'), -i (std::regex::icase), -A (множество индексов строк для вывода; пересекающиеся области объединяются — каждая строка выводится не более одного раза).
- **Интеграция**: `Builtins::cmdGrep` вызывает `runGrep`; регистрация в реестре встроенных команд не меняется (по имени `grep`).
- **Поведение**: вход — stdin или список файлов; выход — строки с совпадениями и контекстом после них; код возврата 0 при наличии совпадения, 1 при отсутствии, 2 при ошибке разбора или невалидном regex.
...
2. создаёт `BuiltinRegistry` и регистрирует `cat/echo/wc/pwd/grep/exit`;
