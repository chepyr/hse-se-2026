...
В рамках текущего этапа (HW2) реализована только базовая часть:

- REPL
- Tokenizer (без `$`-подстановок)
- Parser одной команды (без пайпов)
- Builtins
- ExternalRunner
- Environment
- Executor без pipeline-логики

Механизмы пайпов, `$`-подстановок и полноценный AST пока не реализованы.
